        ...
        for i, data in enumerate(test_dataloader):
            ...
            if opt.codes_from_diffusion:
                ...
                with torch.no_grad():
                    ...
                    ## --- debug [encoded latent] ---
                    images_to_encode = einops.rearrange(data['input'], 'b (h2 w2) c h w -> b c (h2 h) (w2 w)', h2=3, w2=2) 
                    # range (0.,1)
                    # images_to_encode = images_to_encode*2 - 1 # FIXME: ahhhhhhhh
                    # images_to_encode = einops.rearrange(data['input_white'], 'b (h2 w2) c h w -> b c (h2 h) (w2 w)', h2=3, w2=2) 
                    images_to_encode_to_save = einops.rearrange(images_to_encode[0], 'c h w-> h w c')
                    Image.fromarray((images_to_encode_to_save.cpu().numpy() * 255).astype(np.uint8)).save(os.path.join(output_path, f'{name}_enc_white.png'))
                    ...
                    # ### pipe.vae
                    # # codes_vae = model.vae.encode(images_to_encode)
                    # # st()
                    # # image = scale_image(image)
                    # # image = self.vae.encode(image).latent_dist.sample() * self.vae.config.scaling_factor
                    # # image = scale_latents(image)
                    
                    # latents = pipeline_0123.vae.encode(scale_image(images_to_encode)).latent_dist.sample() * pipeline_0123.vae.config.scaling_factor
                    # latents = scale_latents(latents)
                    # print(f"code-pipeline_0123vae-encode: max={latents.max()} min={latents.min()} mean={latents.mean()}")

                    # latents = normalize_to_target(latents, latents)
                    # print(f"Normalized code-pipeline_0123vae-encode: max={latents.max()} min={latents.min()} mean={latents.mean()}")
