...
def main():    
    ...
    if accelerator.is_main_process:
        ...
        for file in ['main_zero123plus_v4_batch_vae.py']:
    ...
    for epoch in range(opt.num_epochs):
        ...
        total_loss_kl = 0 #torch.tensor([0])
        ...
        for i, data in tqdm(enumerate(train_dataloader), total=len(train_dataloader), disable=(opt.verbose_main), desc = f"Training epoch {epoch}"):
            ...
            with accelerator.accumulate(model):
                ...
                #     if param.requires_grad and param.grad is not None:
                #         print(f"Parameter {name}, Gradient norm: {param.grad.norm().item()}")
                # st() # NOTE: All vae.encoder/decoder/quant_conv do have gradients
                ...
                if 'loss_kl' in out.keys():
                    total_loss_kl += out['loss_kl'].detach()
        ...
        if 'loss_kl' in out.keys():
            total_loss_kl = accelerator.gather_for_metrics(total_loss_kl).mean().item()
        ...
        if accelerator.is_main_process:
            ...
            total_loss_kl /= len(train_dataloader)
            
            accelerator.print(f"[train] epoch: {epoch} loss: {total_loss.item():.6f} psnr: {total_psnr.item():.4f} kl_loss: {total_loss_kl:.4f} splatter_loss: {total_loss_splatter:.4f} rendering_loss: {total_loss_rendering:.4f} alpha_loss: {total_loss_alpha:.4f} lpips_loss: {total_loss_lpips:.4f} ")
            ...
            writer.add_scalar('train/loss_kl', total_loss_kl, epoch)
            if opt.log_gs_loss_mse_dict:
                for key in gt_attr_keys:
        ...
        if epoch % opt.eval_iter == 0: 
            ...
            with torch.no_grad():
                ...
                if accelerator.is_main_process:
                    ...
                    total_loss_kl /= len(test_dataloader)
                    
                    accelerator.print(f"[eval] epoch: {epoch} loss: {total_loss.item():.6f} psnr: {total_psnr.item():.4f} kl_loss: {total_loss_kl:.4f} splatter_loss: {total_loss_splatter:.4f} rendering_loss: {total_loss_rendering:.4f} alpha_loss: {total_loss_alpha:.4f} lpips_loss: {total_loss_lpips:.4f} ")
                    ...
                    writer.add_scalar('eval/loss_kl', total_loss_kl, epoch)
