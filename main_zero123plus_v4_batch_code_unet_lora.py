...
from core.models_zero123plus_code_unet_lora import Zero123PlusGaussianCodeUnetLora, gt_attr_keys
...
def main():    
    ...
    opt.model_type = 'Zero123PlusGaussianCodeUnetLora'
    ...
    desc += f"-lora_rank{opt.lora_rank}"
    desc += f"-gpus{num_gpus}"
    ...
    if accelerator.is_main_process:
        ...
        # write lora structure
        after_layers = {name: layer for name, layer in model.unet.named_modules()}
        output_file_path = os.path.join(opt.workspace, 'unet_after_lora.txt')
        # Open the file in write mode
        with open(output_file_path, 'w') as file:
            file.write("Layers after adding LoRA:\n")
            for name in after_layers:
                file.write(f"{name}:\n{repr(after_layers[name])}\n\n")
    ...
    if opt.resume is not None and not opt.decoder_mode.startswith("v1_fix_rgb"):
        ...
        for key, v in ckpt.items():
            ...
            new_key = key
            if new_key not in state_dict: # those has been added lora
                # Update the layer names to the new LoRA-wrapped ones
                if 'to_q.weight' in key:
                    new_key = key.replace('to_q.weight', 'to_q.base_layer.weight')
                elif 'to_k.weight' in key:
                    new_key = key.replace('to_k.weight', 'to_k.base_layer.weight')
                elif 'to_v.weight' in key:
                    new_key = key.replace('to_v.weight', 'to_v.base_layer.weight')
                elif 'to_out.0.weight' in key:
                    new_key = key.replace('to_out.0.weight', 'to_out.0.base_layer.weight')
                elif 'to_out.0.bias' in key:
                    new_key = key.replace('to_out.0.bias', 'to_out.0.base_layer.bias')
                
            # if "lora" in key or "base_layer" in key:
            #     print(key)
            
            if new_key in state_dict: 
                if state_dict[new_key].shape == v.shape:
                    state_dict[new_key].copy_(v)
                else:   
                    accelerator.print(f'[WARN] mismatching shape for param ckpt-{key} and model-{new_key}: ckpt {v.shape} != model {state_dict[new_key].shape}, ignored.')
            else:
                accelerator.print(f'[WARN] unexpected param ckpt-{key}(model-{new_key}): {v.shape}')
            
            ## debug
            del state_dict[new_key]
        
        # st()
        # non_lora_params = [name for name, _ in state_dict.items() if 'lora' in name]
        # print(non_lora_params)
        # # print("non_lora_params that are not in ckpt")
    ...
    for epoch in range(opt.num_epochs):
        ...
        for i, data in tqdm(enumerate(train_dataloader), total=len(train_dataloader), disable=(opt.verbose_main), desc = f"Training epoch {epoch}"):
            ...
            with accelerator.accumulate(model):
                if not opt.codes_from_encoder:
                    ...
                    if num_gpus==1:
                        # codes_before_act_list_grad_, codes, code_optimizers = model.load_scenes(opt.code_cache_dir, data)
                        codes = model.load_scenes(opt.code_cache_dir, data, eval_mode=True)
                    else:
                        # codes_before_act_list_grad_, codes, code_optimizers = model.module.load_scenes(opt.code_cache_dir, data)
                        codes = model.module.load_scenes(opt.code_cache_dir, data, eval_mode=True)
                ...
                if num_gpus > 1:
                    loss_latent = model.module.forward_splatters_with_activation_train_unet(cond=data["cond"], latents=data['codes'])
                else:
                    loss_latent = model.forward_splatters_with_activation_train_unet(cond=data["cond"], latents=data['codes'])
                ...
                # NOTE: CHECKED: decoder and self.scale_bias not have grad, only UNET parameters
                optimizer.step()
                
                # ## check only the lora layers has gradient
                # # Iterate through the model parameters and check for gradients
                # for name, parameter in model.named_parameters():
                #     # Check if the parameter has a gradient
                #     if parameter.grad is not None:
                #         # Check if this is a LoRA layer
                #         if 'lora' in name:
                #             # Print out the norm of the gradient for LoRA layers
                #             print(f"LoRA layer {name} has gradient norm: {parameter.grad.norm().item()}")
                #             pass
                #         else:
                #             # For non-LoRA layers, check if the gradient norm is zero or very close to zero
                #             gradient_norm = parameter.grad.norm().item()
                #             if gradient_norm > 1e-6:  # Use an appropriate threshold to consider as 'no gradient'
                #                 print(f"Unexpected gradient: {name} has gradient norm: {gradient_norm}")
                #             else:
                #                 print(f"Correctly no gradient: {name} has gradient norm: {gradient_norm}")
                # ## NOTE: passed, only lora layer has gradient
                # ## ----- [END] -----
                # st()
                ...
                total_loss += loss_latent.detach()
                total_psnr += torch.tensor([0.]).to(loss_latent.device)
                ...
                if False:
                    ...
                    if not opt.codes_from_encoder:
                        ...
                        if num_gpus==1:
                            model.save_scenes(opt.code_cache_dir, code_list_=codes_before_act_list_grad_, 
                                        scene_names=data['scene_name'],
                                        code_optimizer_list=code_optimizers
                                        )
                        
                        else:
                            model.module.save_scenes(opt.code_cache_dir, code_list_=codes_before_act_list_grad_, 
                                        scene_names=data['scene_name'],
                                        code_optimizer_list=code_optimizers)
        ...
        if epoch > 0 and epoch % opt.save_iter == 0:
            accelerator.wait_for_everyone()
            # accelerator.save_model(model, opt.workspace, safe_serialization=False) # -> pytorch_model.bin
            accelerator.save_model(model, opt.workspace)
            # decoder_params = [name for name, _ in model.named_parameters() if "decoder" in name]
            # print(decoder_params)
            # print("SAVE MODEL: decoder params")
            # st()
            ...
            # accelerator.save_model(model, eval_folder, safe_serialization=False)
            #   print("safe_serialization=False")
            accelerator.save_model(model, eval_folder)
            print("Saved a COPY of new ckpt !!! ")
          
            
            # ## TODO: checked: this should save the whole model, including the lora weights
            # if epoch > opt.save_iter:
            #     loaded_model = load_file(os.path.join(opt.workspace, "model.safetensors"))
            #     # loaded_model = torch.load(os.path.join(opt.workspace, "model.safetensors"))

            #     # Iterate through the loaded model's state dictionary to check for LoRA parameters
            
            #     # decoder_params = [name for name, _ in loaded_model.items() if "decoder" in name]
            #     # print(decoder_arams)
            #     # st()
                
            #     lora_params = [name for name, _ in loaded_model.items() if 'lora' in name]
            #     if lora_params:
            #         print("LoRA adapters have been saved successfully. Found parameters:")
            #         for name in lora_params:
            #             print(name)
            #     else:
            #         print("LoRA adapters have not been found in the saved model.")
                    
            #     st()
                

            
            # ## save lora # TODO(low priority): try this only if the lora works. This only saves the save model size.
