...
from core.models_zero123plus_low_res import Zero123PlusLowRes
from core.dataset_v5_marigold_0123plus import ObjaverseDataset as Dataset
...
def main(): 
    ...
    # Now load the model
    assert opt.model_type == "Zero123PlusLowRes", "Invalid model type"
    model =  Zero123PlusLowRes(opt)
    ...
    if accelerator.is_main_process:
        ...
        for file in ['main_zero123plus_v5_batch_marigold_finetune_0123++_low_res.py']:
                    ...
                    with torch.no_grad():
                        ...
                        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), disable=(opt.verbose_main), desc = f"Eval global_step {global_step}"):
                            ...
                            # if opt.train_unet_single_attr is None:
                                three_in_one = torch.cat([data['images_output'], out['images_pred']], dim=1)
                        ...
                        if accelerator.is_main_process:
                            ...
                            writer.add_scalar('eval/total_loss_other_than_latent', total_loss, epoch)
