...
class SrnCarsDataset(Dataset):
    ...
    def __init__(self, opt: Options, training=True, prepare_white_bg=False):
        ...
        if not opt.data_path_splatter_gt.endswith('cars_train'):
            scene_path_pattern = os.path.join(opt.data_path_splatter_gt, "*", "cars_train", "*")
        else:
            scene_path_pattern = os.path.join(opt.data_path_splatter_gt, "*")
        
        all_scene_paths = sorted(glob.glob(scene_path_pattern))
        print(f"There are total {len(all_scene_paths)} scenes")
        ...
        for scene_path in all_scene_paths:
            ...
            if len(os.listdir(splatter_gt_folder)) == 7:
                ...
                try:
                    assert len(os.listdir(os.path.join(rendering_folder, 'rgb'))) >= 50
                except:
            ...
            if self.opt.overfit_one_scene:
                if len(self.data_path_splatter_gt) > 1:
                    break
    ...
    def __getitem__(self, idx):
        ...
        for vid in vids:
            ...
            c2w[:3, 3] *= 1.5 / 1.3 # 1.5 is the default scale
