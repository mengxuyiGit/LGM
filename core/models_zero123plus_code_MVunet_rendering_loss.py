...
class Zero123PlusGaussianMVDiffusion(nn.Module):
    def __init__(
        self,
        opt: Options,
    ):
    ...
    def forward_splatters_with_activation(self, images, cond, latents=None):
        ...
        if latents == None:
            ...
            # if self.opt.verbose_main:
            print(f"get latent from encoding images of shape: {images.shape}")
            ...
            raise ValueError("Latents should not be loaded from cache")


        gt_latents = latents
        guidance_scale = 1.0
        
        # train both decoder and unet together
        t = torch.randint(0, self.pipe.scheduler.timesteps.max(), (B,), device=latents.device)
        noise = torch.randn_like(latents, device=latents.device)
        noisy_latents = self.pipe.scheduler.add_noise(latents, noise, t)
