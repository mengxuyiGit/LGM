...
class Zero123PlusLowRes(nn.Module):
    def __init__(
        self,
        opt: Options,
    ):
    ...
    def forward(self, data, step_ratio=1, save_path=None, prefix=None, get_decoded_gt_latents=False):
        ...
        if self.opt.lambda_lpips > 0:
            loss_lpips = self.lpips_loss(
                einops.rearrange(gt_images, "b v c h w -> (b v) c h w") * 2 - 1, 
                einops.rearrange(pred_images, "b v c h w -> (b v) c h w") * 2 - 1,
            )
        ...
        if isinstance(loss, int):
            loss = torch.zeros_like(loss_latent)
        results['loss'] = loss
        ...
        st() # stop here. No further gs rendering
