...
class ObjaverseDataset(Dataset):
    ...
    def __init__(self, opt: Options, training=True, prepare_white_bg=False):
        ...
        all_scene_paths_render = sorted(glob.glob(os.path.join(opt.data_path_rendering, "*", "*")))
        for scene_path_render in all_scene_paths_render:
   
            if not os.path.isdir(scene_path_render):
                continue
    
            scene_name = scene_path_render.split('/')[-1]
            if scene_name in self.invalid_objects:
                continue 
            
            self.data_path_rendering[scene_name] = scene_path_render  
               
        all_items = [k for k in self.data_path_rendering.keys()]
        all_items = all_items[opt.scene_start_index : opt.scene_end_index] # 2.5k objs, each 20 views
