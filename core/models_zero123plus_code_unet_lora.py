...
class Zero123PlusGaussianCodeUnetLora(nn.Module):
    def __init__(
        self,
        opt: Options,
    ):
        ...
        # st()
        # print("model(unet) before lora", self.unet)
        # st()
        # # Get the layers before adding LoRA
        # before_layers = {name: layer for name, layer in self.unet.named_modules()}
        # output_file_path = "unet_before_lora.txt"
        # with open(output_file_path, 'w') as file:
        #     file.write("Layers before adding LoRA:\n")
        #     for name in before_layers:
        #         file.write(f"{name}:\n{repr(before_layers[name])}\n\n")
        ...
        # print("model(unet) after lora", self.unet)
        # st()
        # Get the layers after adding LoRA
        after_layers = {name: layer for name, layer in self.unet.named_modules()}

        # Find and print the differences

        # output_file_path = 'unet_after_lora.txt'
        ...
        # output_file_path = 'unet_after_lora_diff.txt'
        # # Open the file in write mode
        # with open(output_file_path, 'w') as file:
        #     file.write("Layers changed by adding LoRA:\n")
        #     for name in after_layers:
        #         if name not in before_layers or repr(before_layers[name]) != repr(after_layers[name]):
        #             print(f"{name}:\n{after_layers[name]}")
        #             file.write(f"{name}:\n{repr(after_layers[name])}\n\n")
        # st()
    ...
    def _reinitialize_unet_weights(self, unet):
        """
        Reinitializes the weights of the U-Net model.
        """
        for module in unet.modules():
            if isinstance(module, (nn.Conv2d, nn.ConvTranspose2d)):
                # Use Kaiming initialization for models with ReLU activations, or Xavier for others
    ...
    def load_scenes(self, code_dir, data, eval_mode=False):
        ...
        for i, scene_name in enumerate(data['scene_name']):
            ...
            cache_file_from_resume = os.path.join(code_dir, scene_name + '.pth')
    ...
    def forward_splatters_with_activation_train_unet(self, cond, latents):
        ...
        if self.opt.one_step_diffusion is not None:
            t = torch.tensor([self.opt.one_step_diffusion]).to(t.device)
            print("ONE STEP DIFFUSION timesteps=",t)
            st()
