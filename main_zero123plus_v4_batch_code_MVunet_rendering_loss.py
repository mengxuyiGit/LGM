...
from core.models_zero123plus_code_MVunet_rendering_loss import Zero123PlusGaussianMVDiffusion
...
def main():    
    ...
    if opt.model_type == 'Zero123PlusGaussian':
        ...
        from core.dataset_v4_code import ObjaverseDataset as Dataset
    elif opt.model_type == "Zero123PlusGaussianMVDiffusion":
        model = Zero123PlusGaussianMVDiffusion(opt)
        from core.dataset_v4_code import ObjaverseDataset as Dataset
    ...
    if accelerator.is_main_process:
        ...
        for file in ['main_zero123plus_v4_batch_code_unet_rendering_loss.py']:
    ...
    for epoch in range(opt.num_epochs):
        ...
        for i, data in tqdm(enumerate(train_dataloader), total=len(train_dataloader), disable=(opt.verbose_main), desc = f"Training epoch {epoch}"):
            ...
            with accelerator.accumulate(model):
                if not opt.codes_from_encoder:
                    st() # NOTE: the MVDiffusion should not use this two stage latent code 
        ...
        if accelerator.is_main_process:
            ...
            writer.add_scalar('train/loss', total_loss_latent.item(), epoch) # for comparison with no rendering loss
        ...
        if epoch % opt.eval_iter == 0: 
            ...
            with torch.no_grad():
                ...
                if accelerator.is_main_process:
                    ...
                    writer.add_scalar('eval/loss', total_loss_latent.item(), epoch)
