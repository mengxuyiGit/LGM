...
class MultiViewNoisedUNet(torch.nn.Module):
    def __init__(self, unet: UNet2DConditionModel, train_sched: DDPMScheduler, val_sched: EulerAncestralDiscreteScheduler) -> None:
        super().__init__()
        self.unet = unet
        self.train_sched = train_sched
        ...
        for name, _ in unet.attn_processors.items():
            if torch.__version__ >= '2.0':
                ...
                default_attn_proc = AttnProcessor()
            unet_lora_attn_procs[name] = MultiViewAttnProc(
                default_attn_proc, enabled=name.endswith("attn1.processor"), name=name
            )
        unet.set_attn_processor(unet_lora_attn_procs)
...
class Zero123PlusPipeline(diffusers.StableDiffusionPipeline):
    ...
    def prepare(self):
        ...
        if isinstance(self.unet, UNet2DConditionModel):
            ...
            self.unet = MultiViewNoisedUNet(self.unet, train_sched, self.scheduler).eval()
