...
class BasicTransformerBlockCrossDomainV1(nn.Module):
    def __init__(
        self,
        dim: int,
        ...
        self.norm_joint_mid = nn.LayerNorm(dim, norm_eps, norm_elementwise_affine)
...
# @maybe_allow_in_graph
class BasicTransformerBlockCrossDomain(nn.Module):
    def __init__(
        ...
        nn.init.zeros_(self.attn_joint_mid.to_out[0].weight.data)
        # st() # check whether the block,.attn1.to_out[0] has been changed: passed!
        # set processor for attn_joint_mid
        processor = XFormersJointAttnProcessor()
        self.attn_joint_mid.set_processor(processor)
        # st()
    ...
    def replace_transformer_blocks(module):
        for name, child in module.named_children():
            if isinstance(child, BasicTransformerBlock):
                ...
                setattr(module, name, BasicTransformerBlockCrossDomain(child))  # configure appropriately
            else:
