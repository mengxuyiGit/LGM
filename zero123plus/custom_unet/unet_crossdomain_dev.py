...
from zero123plus.custom_unet.custom_diffusers.unet_2d_condition_cd import UNet2DConditionModelCDAttn
...
class UNet2DConditionModelCrossDomainAttn(torch.nn.Module):
    def __init__(self, unet: UNet2DConditionModel=None,
        ...
        super().__init__()
        # unet: original unet
        # self.unet: with cross domain Attention
        # with open("unet_zero123++.txt", "w") as f:
        #     print(unet, file=f)
        ...
        # add additinal attention layers for cross domain attention
        
        # add reshaping operations to convert from cross view to cross domain
    ...
    def forward(
        ...
        class_emb = self.unet.get_class_embed(sample=sample, class_labels=class_labels)
        print(f"class_emb: {getattr(class_emb, 'shape', None)}")
        print(f"class_labels: {getattr(class_labels, 'shape', None)}")
        print(f"t_emb: {emb.shape}")
        # st()
        ...
        if self.unet.config.addition_embed_type == "image_hint":
        ...
        if cross_attention_kwargs is not None and cross_attention_kwargs.get("gligen", None) is not None:
            ...
            cross_attention_kwargs["gligen"] = {"objs": self.unet.position_net(**gligen_args)}
