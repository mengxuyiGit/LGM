...
class UNet2DConditionModelChannelwise(torch.nn.Module):
    def __init__(self, unet: UNet2DConditionModel,
        ...
        self.conv_out = nn.Conv2d(
            320, out_channels, kernel_size=3, padding=1
        )

        # copy the weights # NOTE: not to divide the w&b by the factor, to react to the same features
        factor_in =  self.conv_in.in_channels // unet.conv_in.in_channels
        
        # self.conv_in.weight.data = unet.conv_in.weight.data.repeat(1, factor_in, 1, 1)
        self.conv_in.weight.data = unet.conv_in.weight.data.repeat(1, factor_in, 1, 1) / factor_in
        
        factor_out = self.conv_out.out_channels // unet.conv_out.out_channels
        self.conv_out.weight.data = unet.conv_out.weight.data.repeat(factor_out, 1, 1, 1)
        self.conv_out.bias.data = unet.conv_out.bias.data.repeat(factor_out)
