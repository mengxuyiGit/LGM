    ...
    if True:
        ...
        for i, data in enumerate(test_dataloader):
            ...
            if opt.vae_on_splatter_image:
                ...
                # resize the splatter image 
                
                if opt.splatter_size != data["splatters_output"].shape[-1]:
                    print(f"Resizing the splatter image from {data['splatters_output'].shape[-1]} to {opt.splatter_size}")

                    resized_splatter = F.interpolate(data["splatters_output"][0], size=(opt.splatter_size, opt.splatter_size), mode='bilinear', align_corners=False) 
                    print(f"Resized splatter image size: {resized_splatter.shape}")
                    splatters_mv = einops.rearrange(resized_splatter[None], 'b (h2 w2) c h w -> b c (h2 h) (w2 w)', h2=3, w2=2) 
                ...
                if opt.splatter_to_encode is not None:
                    ...
                    # load_path = opt.splatter_to_encode
                ...
                with torch.no_grad():
                    ...
                    for i in tqdm(range(num_iterations), desc='Optimization Progress'):
                        ...
                        splatter_latents_from_diffusion = True
                        if splatter_latents_from_diffusion:
                            for attr in ordered_attr_list:
