...
def main():    
    ...
    if (not opt.codes_from_encoder) or opt.codes_from_cache:
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_dataloader):
            ...
            if opt.codes_from_diffusion:
                ...
                with torch.no_grad():
                    ...
                    # continue
                    diffusion_latents.append(latents)
                    
                    # ## --- debug
                    # code_dir = "runs/zerp123plus_batch/workspace_train/20240315-4gpus_decode_splatter_to_128_nearest-sp_guide_1-codes_lr0.01-v0_unfreeze_all-pred128_last_layer-skip_predict_x0-loss_render1.0_splatter0.5_lpips1.0-lr0.0002-Plat100/code_dir"
                    ...
                    # print("Codes from optimzied ")
                    # # st()
                    # latents = code_[None].to(device=latents.device, dtype=latents.dtype)
                    # data['codes'] = latents
            ...
            if opt.codes_from_cache:
                ...
                # st()
                optimized_latents.append(codes)
            ...
            if opt.codes_from_encoder:
                ...
                # st()
                images_to_encode = einops.rearrange(data['input'], 'b (h2 w2) c h w -> b c (h2 h) (w2 w)', h2=3, w2=2) 
                codes = model.encode_image(images_to_encode)
                print(f"code-encoder: max={codes.max()} min={codes.min()} mean={codes.mean()}")
                encoded_latents.append(codes)
        ...
        # plot the status
        if opt.codes_from_diffusion:
            diffusion_latents_tensor = torch.stack(diffusion_latents, dim=0)
            stats_dict.update({"diffusion":diffusion_latents_tensor})
        if opt.codes_from_cache:
            optimized_latents_tensor = torch.stack(optimized_latents, dim=0)
            stats_dict.update({"optimized":optimized_latents_tensor})
        if opt.codes_from_encoder:
            encoded_latents_tensor = torch.stack(encoded_latents, dim=0)
            stats_dict.update({"encoded":encoded_latents_tensor})
        
        
        # Create a figure and axis to plot on
        plt.figure(figsize=(15, 5))

        for i, (key, tensor) in enumerate(stats_dict.items()):
            # Save the tensor to the specified file
            file_path = f"{key}_tensor.pt"
            torch.save(tensor, file_path) 
            
            print(key, tensor.shape)
            values = tensor.flatten().cpu().numpy()
            plt.subplot(1, 3, i+1)
            plt.hist(values, bins=50, alpha=0.7, label=key)
            plt.title(f'{key} Value Distribution')
            plt.xlabel('Value')
            plt.ylabel('Frequency')
        
        plt.tight_layout()
        plt.savefig(f'{opt.workspace}/latent_distribution.jpg')
        
        # Open a text file for writing
        with open("tensor_stats.txt", "w") as file:
            for i, (key, tensor) in enumerate(stats_dict.items()):
                mean1 = torch.mean(tensor).item()
                std1 = torch.std(tensor).item()
                var1 = torch.var(tensor).item()

                # Write the statistics of the first tensor
                file.write(f"--- {key} ---:\n")
                file.write(f"Tensor Stats:\n")
                file.write(f"Mean: {mean1}\n")
                file.write(f"Standard Deviation: {std1}\n")
                file.write(f"Variance: {var1}\n\n")

        st()
        
        # # plot
        # # Flatten the tensors to get the distribution of all values
        # diffusion_values = diffusion.flatten().numpy()
        # optimized_values = optimized.flatten().numpy()
        # encoded_values = encoded.flatten().numpy()

       
        # # Plot the distribution of each tensor
        # plt.subplot(1, 3, 1)
        # plt.hist(diffusion_values, bins=50, alpha=0.7, label='Diffusion')
        # plt.title('Diffusion Value Distribution')
        # plt.xlabel('Value')
        # plt.ylabel('Frequency')

        # plt.subplot(1, 3, 2)
        # plt.hist(optimized_values, bins=50, alpha=0.7, label='Optimized')
        # plt.title('Optimized Value Distribution')
        # plt.xlabel('Value')
        # plt.ylabel('Frequency')

        # plt.subplot(1, 3, 3)
        # plt.hist(encoded_values, bins=50, alpha=0.7, label='Encoded')
        # plt.title('Encoded Value Distribution')
        # plt.xlabel('Value')
        # plt.ylabel('Frequency')
